...
> **Cold-start note (fresh reader, any LLM, no prior context).** "The Ralph Loop" is `scripts/ralph-loop.sh` at this repo's root — a Bash orchestrator (`set -euo pipefail`) that builds software one *story* at a time, each step a fresh `claude -p` CLI process with no shared chat history (context is reloaded from the file system). Completion is tracked by grepping `git log` for `feat(<id>):` commits. "BMAD" is an agent-skill framework (installed under `.claude/skills/`) whose method goes PRD → Epics → Stories; the loop maps roles to BMAD skills (SM=`bmad-create-story`, Dev=`bmad-dev-story`, Review=`bmad-code-review`, plus planning roles PM/Architect/Planner). "System Track" vs "Demo Track" *was* this repo's two-track split until 2026-07-07: the Demo Track built a sample React app under `src/`; the System Track (this folder, `system/`) improved the loop itself. On 2026-07-07 the Demo Track was retired and the repo renamed Ergane — it is now single-track (the loop plus its self-development), and the Exchange Rates sample now ships through the installer's `--task-source example` rather than living at the repo root (see `system/chapters/2026-07-07-single-track/README.md`). The loop-improvement work this design belongs to continues under `system/`. "Path A intake" is the loop's existing `--issue N` mode, which today reads ONE GitHub issue via the `gh` CLI and turns it into PRD/epic/story files, then builds it — but is read-only against GitHub. `gh` is the GitHub CLI and the *only* GitHub mechanism used (no octokit, no REST client). The full context dossier is §13; the glossary is §12.
...
External dependencies this design assumes (install/verify before implementing): `gh` (GitHub CLI, authenticated with **write** scope), `git` (incl. `git worktree`), `jq`, a small **TypeScript/Node** runtime for the reconciler (§8 — a deliberate exception, alongside `installer/`, to the repo's "Bash + Markdown only" rule; the CLAUDE.md amendment noted in §8 has since landed), and the BMAD planning/build skills listed in §10.
...
- **Who is acting:** Seevali — owner/maintainer of `seevali/ergane` (a public repo, single-track since 2026-07-07: the repo *is* the Ergane loop product; it still sits under Metis's `demos/` folder locally — relocating it is a flagged future cleanup). He may switch LLM providers, hence the cold-start discipline.
