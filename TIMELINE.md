...
## Single Track — the demo retires, the loop gets a name: Ergane (2026-07-07 → 2026-07-08)

The repo's founding structure — a clonable Demo Track (the Exchange Rates Dashboard React app + PRD at the root) alongside the System Track lab — was dissolved, on one decisive piece of evidence: **the demo had never actually been run.** `src/` still held the vanilla Vite scaffold, `docs/stories/` was empty; the showcase was a promise, not a record, and the installer had matured into a better vehicle for the same promise. The owner's call: *reserve the repo entirely for the loop.* Full decision ledger (D1–D8) in the [single-track chapter](system/chapters/2026-07-07-single-track/README.md).

**The name.** The product is now **Ergane** — Athena Ergane, "the Worker," patroness of craftsmen; in myth Metis is Athena's mother, which is precisely this tool's relationship to the Metis monorepo it was born in. A clearance pass found it the only fully-clean candidate (npm scoped + unscoped free, `seevali/ergane` free, no dev-tool or trademark collisions; `telos` and `talos` were hard blockers, `automedon` a confusion risk). Brand-level rename only (`5e2869d`): repo, npm identity (`@seevali/ergane`), docs, installer copy. Engine names — `scripts/ralph-loop.sh`, `RALPH_*`, `.ralph/` — deliberately keep the pattern's name: *Ergane is the product; its engine speaks Ralph.*

**Workload-neutrality (`25e9e6c`).** The loop's demo defaults are gone: `--project-dir`, `--epic` (Path B), and `--checkpoint` are required flags with honest errors. Layer 3a of every agent prompt now reads the target project's own `docs/project-conventions.md` — the file the installer had always rendered per-target *and the loop had never read* (a real bug, fixed); the review overlay's hardcoded `cd src && npm run build` gave way to `{{CHECKPOINT_CMD}}`. Adversarial review earned its keep again: a reviewer proved the rewritten staging logic would have `git add`-ed the **entire repo** on every System Track story commit (`--project-dir .` → `$PROJECT_DIR == $REPO_ROOT`), caught before it ever ran live; also caught — stale conventions resolution after a worktree re-point, and shipped docs whose example commands would have failed the new required flags.

**The example travels the install path (`725c398`).** The demo's one genuinely valuable artifact — the fully-authored PRD + epic — moved into the installer as `--task-source example`: a fresh repo gets a ready-to-run demo through the *real* install path, which is a better showcase than a frozen clone ever was. The verifier's decisive gate: the exact command GETTING-STARTED documents runs verbatim (exit 0). Reviewer catches: the example checkpoint now derives from the live app-dir answer instead of hardcoding `cd src`; doctor's epic validation became manifest-driven (it had silently skipped non-scaffold epics) and stopped telling users to "run update" for user-owned files update structurally never touches. Installer suite 288 → 301.

**The strip (`c339578`).** `src/` and the root demo docs deleted (−5,316 lines); `docs/github-issue-intake-prompt.md` relocated to `system/design/` (it was System Track design history all along); README rewritten as a tool README with an honest install story (npm publish still pending — the `npx` path is labeled aspirational, the clone path verified live); one CLAUDE.md rule set (bash + markdown, Node in `installer/`+`tools/`); this header's two-track framing retired. Historical entries, closed chapters, and `todo.md` stand untouched — the record matters more than tidiness. A blog banner + sequel are drafted as chapter artifacts; publishing them (and regenerating the audio) is the owner's call, as is the still-open live `--write` dogfood run (planned for the weekend of 2026-07-11) and the npm publish itself.

[Chapter](system/chapters/2026-07-07-single-track/README.md) | commits `5e2869d`, `25e9e6c`, `725c398`, `c339578`

---

## [System] Installer UX Refresh — audit-first, two slices, 227→288 tests (2026-07-04)

Opened and closed the [Installer UX Refresh chapter](system/chapters/2026-07-04-installer-ux-refresh/README.md) the same day the Round-Trip chapter shipped — because shipping it created measurable freshness debt: the installer's synced loop template went stale (the `sync-templates.sh --check` hard-block gate turned red, 1 installer e2e failing), and an installed project would have gotten a loop advertising swarm/brake features whose companion script `ralph-watch.sh` the installer never shipped. The owner's ask on top: *make installation an intuitive experience.*

**Audit before build:** four read-only UX auditors drove the real CLI in sandboxes (fresh-user, lifecycle, error/edge, docs/copy) and returned **34 evidenced findings — 5 blockers, 11 major — plus 27 delights to protect**. The blockers were damning in a useful way: the outro's own "next command" errored on a fresh install; every doctor/update instruction shipped the literal placeholder `npx <package>`; non-interactive uninstall crashed mid-delete; `update` silently replaced user config with wizard defaults; a corrupt manifest produced three different wrong behaviors. Full evidence lives in the chapter (`ux-audit-findings.json`).

**Slice 1 — freshness + first-run experience (`daa4eab`):** templates resynced with the drift gate widened to three targets (loop, `ralph-watch.sh`, prompts incl. the planning layers); `ralph-watch.sh` scaffolded/doctored/updated/uninstalled like a first-class file; the outro became an honest ordered path (cd → *author the scaffolded PRD/epic by name* → run the loop against files that exist → where progress appears) with a calm token-cost note and budget knobs; GETTING-STARTED rewritten truthfully for both workflows, with a doc-anti-drift test that greps the guide's variable names against the shipped script. **Slice 2 — lifecycle correctness (`661ca64`):** uninstall made crash-free, re-runnable, and respectful of user-created directories (a new `createdDirs` manifest field — a reviewer catch); update preserves user config and can say "Already up to date"; one shared manifest loader with honest errors; the BMAD step probes bmad-method's real flags, fails honestly, and prints a remediation identical to what it ran; `.gitignore` no longer hard-fails real projects; doctor separates "customized" (INFO) from "drifted" (FAIL + remediation); preflight checks the ground (writable target, git identity, gh) before the wizard spends the user's time.

**Method (same as the round-trip chapter):** orchestrator specs → Opus implementer → two adversarial reviewers → fixer → independent re-verification per slice. Installer suite: 227 (1 red) → **288/288**; version bumped to 0.2.0 (npm publish left as a human decision). One orchestration story worth keeping: the contract auditor flagged the orchestrator's own chapter folder as a work-surface violation and the fixer deleted it — the reviewers enforcing the contract against their operator's files is the system working; the folder was restored outside the slice.

[Chapter](system/chapters/2026-07-04-installer-ux-refresh/README.md) | [audit evidence](system/chapters/2026-07-04-installer-ux-refresh/ux-audit-findings.json) | commits `daa4eab`, `661ca64`
...
**How it works:** `run_triage_phase()` fetches the issue (an ungated read) and classifies it with `triage_classify()` — a **pure, deterministic bash scorer** (no LLM call): structure, length, acceptance-criteria/repro signals, and question-shaped titles produce `ready` | `needs-info` | `wontfix-candidate` | `excluded`. Determinism is the point — the issue's AC demands "deterministic given the issue content," which an LLM scorer cannot promise; an LLM triage persona remains open as a possible later layer *on top of* the gate. Dispositions: **ready** → `ralph:ready`, promoted into Phase 0; **needs-info** → specific clarifying questions posted as ONE self-updating issue comment (its own `RALPH:TRIAGE` HTML-comment fences, fail-closed like the slice-c build comment) + `ralph:needs-triage`, then the run parks (exit 2) — ask, don't guess; **wontfix-candidate** → `ralph:blocked` + an explanatory comment, parked — the loop **never closes** an issue (ADR-001 I3); **excluded** (`roadmap` label) → parked with **zero writes** — the loop must not touch its own planning issues (the dogfooding-recursion guard, prd.md §6); a human promotes those deliberately by adding `ralph:ready`.

**Invariants:** all writes funnel through the slice-1 `gh_*_op` guards and go dry under `--write` off — but the *gate still gates* (classification is logged, unready issues still don't build; `--triage never` is the explicit bypass, `--triage always` re-scores a promoted issue). The three stage labels join the single mutually-exclusive `ralph:` status namespace (`RALPH_STATUS_LABELS` 4 → 7), preserving the "exactly one `ralph:` label at any time" projection; re-runs converge with zero label churn (I2). Each triage decision appends to a local, gitignored ledger (`.ralph/triage-ledger.tsv`) — the raw data for the triage-precision metric and its kill criterion (prd.md §7). The safety contract (`run_claude()`, routing, retry, budgets, `main()`) is untouched byte-for-byte, and `--dry-run-prompts` stays byte-identical to the golden.

**How it was built (a first for this repo):** not by hand and not by the loop, but by a **supervised multi-agent orchestration session** — a spec written by the orchestrating agent, one implementer agent, two parallel adversarial reviewers (an ADR-invariant auditor and a bash edge-case hunter), and a fixer. The reviewers earned their keep: they caught the offline smoke disabling `errexit` around the very functions it claimed to prove (a false-green class) and substring keyword matches (`repro` in "reprogram") that could promote underspecified issues — both fixed before commit. Offline smoke: 10/10; all six prior smokes stay green. See the chapter's [BUILD-JOURNAL](system/chapters/2026-06-25-github-issue-roundtrip/BUILD-JOURNAL.md).

[Issue #2 spec](system/chapters/2026-06-25-github-issue-roundtrip/issues/04-triage-before-toil.md) | [smoke](system/chapters/2026-06-25-github-issue-roundtrip/tests/idea4-triage-smoke.sh) | commit `9f070ae`
