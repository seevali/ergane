 ...
 * @returns {string} e.g. "@seevali/ralph-loop"
 */
export function getPackageName() {
  try {
    const pkgPath = path.join(__dirname, '..', 'package.json');
    const name = JSON.parse(readFileSync(pkgPath, 'utf8')).name;
    return typeof name === 'string' && name.length > 0 ? name : '@seevali/ralph-loop';
  } catch {
    return '@seevali/ralph-loop';
  }
}

/**
 * The `npx <name>` invocation users should type to run the CLI.
 * @returns {string} e.g. "npx @seevali/ralph-loop"
 */
