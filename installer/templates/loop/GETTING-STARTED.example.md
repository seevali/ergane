...
You chose to install the **worked example** — the *Exchange Rates Dashboard*, a small
React + Vite + TypeScript app. Unlike the scaffold path, **this plan is already
authored**: a complete PRD and a six-story epic ship ready to build.
**There are no TODOs to fill in** — you can run the loop right now and watch it build.

This guide gets you from this fresh install to your first autonomous build, then shows
the two ways to feed the loop work: **from an epic file** (the quick-start path you're
on) and **from GitHub issues** (the issue-driven path).
...
## Quick start — build the example (Path B)

This is the fastest path to a first success. The plan is done; you just run the loop.

### 1. Review the example plan (optional — it's ready as-is)

The installer wrote two files:

- **`docs/prd.md`** — the product-requirements doc for the Exchange Rates Dashboard. The Scrum-Master agent reads it to write story specs.
- **`docs/epics/exchange-rates-dashboard.md`** — the epic: six stories (`### Story 1.1` … `### Story 1.6`) that build the dashboard end to end, from the app shell to a live rates chart.

Both are **complete and ready to run** — no placeholders to replace. Skim them if you
like, then run the loop. (They're yours now: edit them freely; `update` won't clobber them.)
...
`--checkpoint` is **required** — it is the shell command the loop runs after every story to prove the project still builds and passes tests, and every review step gates on it. The value above is the checkpoint the example install chose (`cd {{APP_DIR}}` first, because the app lives in `{{APP_DIR}}/`); change it any time. The loop bakes in no default, so a command without `--checkpoint` stops immediately with `Error: --checkpoint is required`.

Your app source lives in `{{APP_DIR}}/` (the installer created it empty with a `.gitkeep`; Story 1.1 stands up the Vite app there). The loop, for each story, will:
...
To build just the first story and watch it land, pass `--stories 1.1`.
...
The loop makes **paid Anthropic API calls** on every story. A small story typically costs **cents to a few dollars**; the full six-story example run adds up. The installed defaults are deliberately generous, so set a cap before your first long run.
...
A safe first run — build only Story 1.1, capped at $2:
...
Instead of building the shipped epic, you can point the loop at a GitHub issue and let it plan and build. This path needs the **GitHub CLI (`gh`)** installed and authenticated (`gh auth login`). Run `{{CLI_INVOCATION}} doctor` to check — it reports `gh` status as informational.
...
It checks that all required files are present and unmodified, that `jq` and `claude` are on your PATH, that `scripts/ralph-watch.sh` is present and executable, that the example epic's story headers parse, and (informational) whether `gh` is installed and authenticated for the issue workflow.
